...
Get rid of the spacing between the rows, the color differences will differentiate them enough

### Prompt 20

put a darker grey border on the tool uses row

### Prompt 21

Undo the top and bottom border changes. i meant adding a side border like how the assistant and user prompts show

### Prompt 22

On the bottom of the tool row, there is a little white gap of a few pixels. get rid of that so that it's flush against the neighboring event
...
Now, for the user and assistant event rows, we are wasting space by having "user" and "assistant" on their own lines. For these types of rows, take the first sentence of the text and display it, bolded, to the right of "user" or "assistant". If the sentence is too long, display only some part of it, then an ellipse, then the rest of the sentence in the paragraph underneath.

### Prompt 25

Instead of just the first line, take a minimum of 75 characters to display on the first row.

### Prompt 26

format the markdown properly in the header rows for user and assistant events
